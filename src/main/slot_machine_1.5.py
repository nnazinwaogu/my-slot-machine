    ...
    while True:
        ...
        if int(userInput) != 1 and int(userInput) != 2:
            ...
            while True:
                ...
                intBet = int(bet)
                if intBet > currbalance or intBet <= 0:
                    print("Haha! You can't do that!\n")
                    continue
                else:
                    currbalance, netpos, spins, wagered = lines(currbalance, netpos, spins, wagered, intBet) #updating modified variables
                    break
            continue
    return startingbalance, currbalance, netpos, spins, wagered
    
def fruitTupleBuilder(fruit, value): #added a builder function for ease of adding new fruits
    return tuple((fruit, value))
...
def lines(currbalance, netpos, spins, wagered, bet): #want to separate the slot function logic from the betting logic 
    ...
    apple = fruitTupleBuilder("Apple", bet) #set the value of fruits to the wager amount to add to its significance, 1.5.1 patch 1  
    orange = fruitTupleBuilder("Orange", bet)
    banana = fruitTupleBuilder("Banana", bet)
    lines = [[apple, orange, banana], 
             [orange, banana, apple],
             [banana, apple, orange],]
    multiplerArray = [1, 2, 1, 4, 1, 6, 1, 8, 1, 10, 1, 12]

    #Random number generation for spin logic    
    ...
    if (result1[0]) == (result2[0]) == (result3[0]):
        overall_result = (result1[1]) + (result2[1]) + (result3[1]) 
        print(f"You've spun a: {result1[0]}, {result2[0]}, and {result3[0]}!\nWinnings: ${overall_result}\n")
        ...
        print(f"You've spun: {result1[0]}, {result2[0]}, {result3[0]}, must spin 3 in a row!\n")
        currbalance += overall_result
        print(f"New wallet balance is: ${currbalance}\n")
        netpos += overall_result 
        spins += 1
